...
use reveal_gestures::{GestureTapCancelCallback, GestureTapDownCallback, GestureTapUpCallback};
use reveal_painting::{AnyColor, EdgeInsets, EdgeInsetsGeometry, TextOverflow, TextStyle};
...
impl CupertinoTextSelectionToolbarButtonState {
    fn on_tap_down(self: Handle<Self>, app: &mut App, _details: reveal_gestures::TapDownDetails) {
        self.set_state(app, |state| state.is_pressed = true);
    }

    fn on_tap_up(self: Handle<Self>, app: &mut App, _details: reveal_gestures::TapUpDetails) {
