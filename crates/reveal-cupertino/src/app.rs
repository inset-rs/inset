...
use reveal_embedder::{Brightness, Locale, SystemUiOverlayStyle, TargetPlatform};
use reveal_foundation::{App, Handle};
use reveal_gestures::MultitouchDragStrategy;
...
impl CupertinoApp {
    ...
    pub fn on_generate_route(
        mut self,
        on_generate_route: impl Fn(&mut App, &reveal_widgets::RouteSettings) -> Option<AnyRoute>
        + 'static,
    ) -> CupertinoApp {
    ...
    pub fn on_unknown_route(
        mut self,
        on_unknown_route: impl Fn(&mut App, &reveal_widgets::RouteSettings) -> Option<AnyRoute>
        + 'static,
    ) -> CupertinoApp {
...
mod tests {
    ...
    use reveal_embedder::{Color, Platform, PlatformRef, ViewId, ViewRef};
        ...
        mount(&mut app, |cupertino_app| {
            cupertino_app
                ...
                .on_generate_route(move |app, settings| {
                    ...
                    let route = CupertinoPageRoute::new(app, builder);
                    Some(reveal_widgets::Route::as_route(route))
                })
        });
