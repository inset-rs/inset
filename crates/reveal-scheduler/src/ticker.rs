...
pub trait TickerProvider {
    ...
    fn create_ticker(&mut self, app: &mut App, on_tick: TickerCallback) -> Handle<Ticker>;
}

/// Calls its callback once per animation frame, when enabled.
///
/// When created, a ticker is initially disabled. Call
/// [`start`](TickerMethods::start) to enable the ticker.
///
/// A [`Ticker`] can be silenced by setting muted to true. While silenced, time
/// still elapses, and [`start`](TickerMethods::start) and
/// [`stop`](TickerMethods::stop) can still be called, but no callbacks are
/// called.
///
/// By convention, the [`start`](TickerMethods::start) and
/// [`stop`](TickerMethods::stop) methods are used by the ticker's consumer
...
impl Ticker {
    ...
    /// [`start`](TickerMethods::start) is called and false when
    /// [`stop`](TickerMethods::stop) is called.
    ///
    ...
    /// use [`is_ticking`](TickerMethods::is_ticking).
...
/// Dart's private members of `Ticker`, on the handle.
trait TickerPrivateMethods {
    /// Whether a tick should be scheduled.
    ///
    /// If this is true, then calling
    /// [`schedule_tick`](TickerPrivateMethods::schedule_tick) should succeed.
    ///
    /// Reasons why a tick should not be scheduled include:
    ///
    /// * A tick has already been scheduled for the coming frame.
    /// * The ticker is not active ([`start`](TickerMethods::start) has not
    ///   been called).
    /// * The ticker is not ticking, e.g. because it is muted (see
    ///   [`is_ticking`](TickerMethods::is_ticking)).
    ...
    /// [`should_schedule_tick`](TickerPrivateMethods::should_schedule_tick) is
    /// true.
    fn schedule_tick(self, app: &mut App, rescheduling: bool);

    /// Cancels the frame callback that was requested by
    /// [`schedule_tick`](TickerPrivateMethods::schedule_tick), if any.
    ///
    /// Calling this method when no tick is scheduled is harmless.
    fn unschedule_tick(self, app: &mut App);
}

impl TickerPrivateMethods for Handle<Ticker> {
    fn should_schedule_tick(self, app: &mut App) -> bool {
        !app.get(self).muted && app.get(self).is_active() && !app.get(self).scheduled()
    }

    fn tick(self, app: &mut App, time_stamp: Duration) {
        debug_assert!(self.is_ticking(app));
        debug_assert!(app.get(self).scheduled());
    ...
    fn schedule_tick(self, app: &mut App, rescheduling: bool) {
        ...
        let animation_id = SchedulerBinding::schedule_frame_callback(
            app,
            FrameCallback::handle_method(self, Self::tick),
            rescheduling,
            false,
        );
...
///
/// The [`TickerMethods::start`] method returns a [`TickerFuture`]. The
/// [`TickerFuture`] will complete successfully if the [`Ticker`] is stopped
/// using [`TickerMethods::stop`] with `canceled` false.
///
...
///
/// [`when_complete`]: TickerFutureMethods::when_complete
/// [`when_complete_or_cancel`]: TickerFutureMethods::when_complete_or_cancel
...
impl TickerFuture {
    ...
    pub fn complete(app: &mut App) -> Handle<TickerFuture> {
        let this = app.create(TickerFuture::new());
        TickerFuture::mark_complete(app, this);
        this
    }

    /// Dart's private `_complete`, called by [`TickerMethods::stop`].
    fn mark_complete(app: &mut App, this: Handle<TickerFuture>) {
        debug_assert!(app.get(this).completed.is_none());
        let future = app.get_mut(this);
    ...
    /// Dart's private `_cancel`, called by [`TickerMethods::stop`] with
    /// `canceled` and by [`TickerMethods::dispose`].
    fn mark_canceled(app: &mut App, this: Handle<TickerFuture>) {
        debug_assert!(app.get(this).completed.is_none());
        let future = app.get_mut(this);
...
/// The method surface of Dart's `Ticker`, on the handle.
pub trait TickerMethods {
    ...
    fn set_muted(self, app: &mut App, value: bool);
    ...
    fn is_ticking(self, app: &mut App) -> bool;
    ...
    /// [`start`](TickerMethods::start) function of the original [`Ticker`]
    /// if the original ticker is active.
    ///
    /// This ticker must not be active when this method is called.
    fn absorb_ticker(self, app: &mut App, original_ticker: Handle<Ticker>);
    ...
    ///   [`schedule_tick`](TickerPrivateMethods::schedule_tick), if any, is
    ///   canceled.
    /// * The future that was returned by [`start`](TickerMethods::start)
    ///   does not resolve.
    /// * Callbacks registered with
    ///   [`TickerFutureMethods::when_complete_or_cancel`] resolve as
    ///   canceled.
    fn dispose(self, app: &mut App);
}

impl TickerMethods for Handle<Ticker> {
    fn set_muted(self, app: &mut App, value: bool) {
    ...
    fn is_ticking(self, app: &mut App) -> bool {
        let ticker = app.get(self);
        if ticker.future.is_none() {
            return false;
        }
        if ticker.muted {
            return false;
    ...
    fn start(self, app: &mut App) -> Handle<TickerFuture> {
        ...
        let future = app.create(TickerFuture::new());
    ...
    fn stop(self, app: &mut App, canceled: bool) {
        ...
        debug_assert!(!app.get(self).is_active());

        self.unschedule_tick(app);
        if canceled {
            TickerFuture::mark_canceled(app, local_future);
        } else {
            TickerFuture::mark_complete(app, local_future);
        }
    }
...
/// The method surface of Dart's `TickerFuture`, on the handle.
pub trait TickerFutureMethods {
    ...
    fn when_complete(self, app: &mut App, callback: Listener);

    /// Calls `callback` either when this future resolves or when the ticker
    /// is canceled.
    fn when_complete_or_cancel(self, app: &mut App, callback: Listener);
}

impl TickerFutureMethods for Handle<TickerFuture> {
    fn when_complete(self, app: &mut App, callback: Listener) {
...
mod tests {
    ...
    fn a_ticker_ticks_with_elapsed_time_until_stopped() {
        ...
        let ticker = app.create(Ticker::new(elapsed_recorder(&log)));

        ticker.start(&mut app);
        assert!(app.get(ticker).is_active());
    ...
    fn a_ticker_is_slowed_by_time_dilation() {
        ...
        let ticker = app.create(Ticker::new(FrameCallback::new({
    ...
    fn the_ticker_future_completes_through_the_microtask_queue() {
        let mut app = App::new();
        let ticker = app.create(Ticker::new(FrameCallback::new(|_app, _t| {})));
    ...
    fn absorbing_a_ticker_keeps_the_original_future_alive() {
        let mut app = App::new();
        let log = Rc::new(RefCell::new(Vec::new()));
        let original = app.create(Ticker::new(elapsed_recorder(&log)));
        let future = original.start(&mut app);

        let absorber = app.create(Ticker::new(FrameCallback::new(|_app, _t| {})));
        absorber.absorb_ticker(&mut app, original);

        assert!(app.get(absorber).is_active());
