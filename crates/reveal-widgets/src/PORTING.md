...
- Change: the tweens this crate declares are `Clone` values built without the `App`, where `reveal-animation`'s are arena objects.
...
- Change: the root bucket is read within the call, so what Dart runs from the future's continuation runs inline.
  Reason: platform — the host answers `Platform::restoration_get` within the call (reveal-services' `root_bucket`).
...
- overlay.rs: the theater's clip layer handle and the entry location's compositing-bits update. Trigger: layer handles and compositing bits in `reveal-rendering`.
- overlay.rs: the deferred-child gates on the theater's `markNeedsLayout` and the layout box's dirty flag. Trigger: a public `RenderBox` super-hook for `markNeedsLayout` and a readable dirty flag in `reveal-rendering`.
