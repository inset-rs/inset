...
use reveal_animation::{Animatable, AnyAnimation};
use reveal_embedder::{Matrix4, Offset, Rect, Size, TextAlign, TextDirection};
use reveal_foundation::{App, Handle, Listenable, Listener};
use reveal_painting::{Alignment, AlignmentGeometry, Axis, Decoration, TextOverflow, TextStyle};
...
///
/// Unlike the tweens in `reveal_animation`, this one is a value rather than an arena
...
mod tests {
    ...
    use reveal_animation::{AnimationBehavior, AnimationController, CallbackAnimatable};
    use reveal_embedder::Color;
    use reveal_foundation::{ChangeNotifier, ValueNotifier};
    use reveal_painting::{AnyColor, BoxDecoration};
    use reveal_rendering::{
    ...
    fn under_root<T: reveal_rendering::RenderObject>(
        harness: &Harness,
        app: &App,
    ) -> RenderHandle<T> {
        harness
    ...
    fn a_size_transition_clips_its_child_to_the_size_factor() {
        ...
        harness.pump(&mut app);
        let clip: RenderHandle<reveal_rendering::RenderClipRect> = under_root(&harness, &app);
    ...
    fn a_positioned_transition_places_its_child_by_the_relative_rect() {
        ...
        let stack: RenderHandle<reveal_rendering::RenderStack> = under_root(&harness, &app);
    ...
    fn a_decorated_box_transition_hands_the_animated_decoration_to_its_render_object() {
        ...
        let decorated: RenderHandle<reveal_rendering::RenderDecoratedBox> =
    ...
    fn an_align_transition_hands_the_animated_alignment_to_its_render_object() {
        ...
        let positioned: RenderHandle<reveal_rendering::RenderPositionedBox> =
            under_root(&harness, &app);
        assert_eq!(
