...
///
/// Rust has no subtyping, so a flight holds this as `Rc<dyn RectTweenObject>`; reveal-animation's
...
///
/// reveal-animation's `ReverseTween` wraps a concrete `Tween<T>`, which the erased tween a
...
mod tests {
    ...
    use reveal_painting::{AlignmentGeometry, EdgeInsets, EdgeInsetsGeometry};
