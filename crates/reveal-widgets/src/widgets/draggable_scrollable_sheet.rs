...
/// [`Axis`](reveal_painting::Axis) of the containing area.
///
...
/// [`Listenable`](reveal_foundation::Listenable). It notifies its listeners whenever an
...
impl DraggableScrollableSheetScrollPosition {
    ...
    pub fn list_should_scroll(self: Handle<Self>, app: &App) -> bool {
        reveal_rendering::ViewportOffset::pixels(self, app) > 0.0
    }
...
impl reveal_rendering::ViewportOffset for DraggableScrollableSheetScrollPosition {
...
mod tests {
    ...
    fn a_drag_on_the_scrollable_grows_the_sheet_up_to_the_max_child_size() {
        ...
        let drag = ScrollPosition::drag(
            position,
            &mut app,
            reveal_gestures::DragStartDetails::default(),
            Listener::new(|_app| {}),
        );
        ...
        assert_eq!(
            reveal_rendering::ViewportOffset::pixels(position, &app),
            0.0
        );
        ...
        assert_eq!(controller.size(&app), 1.0);
        assert!(reveal_rendering::ViewportOffset::pixels(position, &app) > 0.0);
