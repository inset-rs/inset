...
/// An inherited widget for a [`Listenable`](reveal_foundation::Listenable) [`notifier`](Self::notifier), which updates its
...
/// See also:
///
///  * `Animation`, an implementation of [`Listenable`](reveal_foundation::Listenable) that ticks each frame to
///    update a value.
///  * [`ViewportOffset`](reveal_rendering::ViewportOffset) or `ScrollPosition`,
///    implementations of [`Listenable`](reveal_foundation::Listenable) that trigger when a view is scrolled.
...
pub trait InheritedNotifier: InheritedWidget {
    ...
    /// The [`Listenable`](reveal_foundation::Listenable) object to which to listen.
    ///
