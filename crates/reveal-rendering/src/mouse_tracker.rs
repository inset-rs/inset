...
use reveal_services::{MouseCursorManager, MouseCursorRef, SystemMouseCursors};
...
/// [`MouseTrackerAnnotation`](reveal_services::MouseTrackerAnnotation), notified by [`update_with_event`] and
...
mod tests {
    ...
    fn tree_with_region(app: &mut App) -> RegionTree {
        ...
        let padding = RenderPadding::new(
            app,
            reveal_painting::EdgeInsets::all(50.0).into(),
            None,
            Some(region.as_box()),
        );
    ...
    fn a_still_mouse_notices_the_tree_moving_under_it_after_a_frame() {
        ...
        padding.set_padding(
            &mut app,
            reveal_painting::EdgeInsetsGeometry::only(150.0, 0.0, 0.0, 0.0),
        );
