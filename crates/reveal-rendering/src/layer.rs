...
use reveal_foundation::{App, Handle, HandleId, RetainedHandle};
...
pub struct ClipPathLayer {
    layer: LayerData,
    container_layer: ContainerLayerData,
    clip_path: Option<Arc<reveal_embedder::Path>>,
    clip_behavior: Clip,
}
...
impl ClipPathLayer {
    ...
    /// The scene must be explicitly recomposited after this property is changed (as described at
    /// [`Layer`]).
    pub fn clip_path(self: Handle<Self>, app: &App) -> Option<&Arc<reveal_embedder::Path>> {
    ...
    pub fn set_clip_path(
        self: Handle<Self>,
        app: &mut App,
        value: Option<Arc<reveal_embedder::Path>>,
    ) {
...
mod tests {
    ...
    fn a_picture_layer_is_a_leaf_that_holds_its_picture() {
        ...
        let picture = Arc::new(reveal_embedder::Canvas::new().build());
    ...
    fn clip_layers_reject_annotations_outside_the_clip() {
        ...
        rrect.set_clip_rrect(
            ...
            Some(RRect::from_rect_and_radius(
                ...
                reveal_embedder::Radius::circular(4.0),
            )),
        );
