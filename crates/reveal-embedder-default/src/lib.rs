...
use reveal_embedder::EmbedderClient;

#[cfg(not(target_arch = "wasm32"))]
pub use reveal_embedder_winit::ImplicitViewConfig;

/// Runs the application on this target's host.
#[derive(Default)]
pub struct DefaultEmbedder {
    #[cfg(not(target_arch = "wasm32"))]
    inner: reveal_embedder_winit::WinitEmbedder,
    #[cfg(target_arch = "wasm32")]
    inner: reveal_embedder_web::WebEmbedder,
}
...
impl DefaultEmbedder {
    ...
    pub fn run<C: EmbedderClient + 'static>(
        self,
        start: impl FnOnce(reveal_embedder::PlatformRef) -> C + 'static,
    ) {
