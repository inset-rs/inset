...
use inset_rendering::{AnyRenderObject, RenderAnnotatedRegion, RenderBox, RenderHandle};
...
/// See also:
///
///  * [`Layer::find`](inset_rendering::AnyLayer::find), for how this value is
///    retrieved.
///  * [`AnnotatedRegionLayer`](inset_rendering::AnnotatedRegionLayer), the layer pushed into
///    the recording.
#[derive(Debug)]
pub struct AnnotatedRegion<T> {
    pub key: Option<KeyRef>,
    /// A value which can be retrieved using
    /// [`AnyLayer::find`](inset_rendering::AnyLayer::find).
    ///
    ...
    ///
    /// An [`AnnotatedRegionLayer`](inset_rendering::AnnotatedRegionLayer) with a size checks
    /// that the offset provided in [`AnyLayer::find`](inset_rendering::AnyLayer::find)
