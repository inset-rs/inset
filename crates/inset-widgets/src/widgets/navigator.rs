...
use inset_embedder::{Clip, RestorationData, RestorationMap};
...
impl inset_foundation::ChangeNotifier for History {
    ...
    fn build(self: Handle<Self>, app: &mut App, context: BuildContext) -> WidgetRef {
        ...
        let listener = crate::widgets::basic::Listener::new()
            .on_pointer_down(Rc::new(
                move |app, event: inset_gestures::PointerDownEvent| {
                    self.handle_pointer_down(app, event.pointer);
                },
            ))
            .on_pointer_up(Rc::new(
                move |app, event: inset_gestures::PointerUpEvent| {
                    self.handle_pointer_up_or_cancel(app, event.pointer);
                },
            ))
            .on_pointer_cancel(Rc::new(
                move |app, event: inset_gestures::PointerCancelEvent| {
                    self.handle_pointer_up_or_cancel(app, event.pointer);
                },
            ))
            .child(absorb);
...
impl inset_foundation::ChangeNotifier for HistoryProperty {
    fn change_notifier_data(&self) -> &inset_foundation::ChangeNotifierData {
        &self.change_notifier
    }
...
impl inset_foundation::ChangeNotifier for RestorableRouteFuture {
    fn change_notifier_data(&self) -> &inset_foundation::ChangeNotifierData {
        &self.change_notifier
    }
...
mod tests {
    ...
    fn corner_page() -> RoutePageBuilder {
        Rc::new(|_app, _context, _animation, _secondary| {
            Align::new()
                .alignment(inset_painting::AlignmentGeometry::TOP_LEFT)
