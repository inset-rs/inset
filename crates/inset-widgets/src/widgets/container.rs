...
use inset_embedder::{Clip, Matrix4, Offset, Path, Size, TextDirection};
use inset_foundation::App;
use inset_painting::{AlignmentGeometry, AnyColor, Decoration, EdgeInsetsGeometry};
use inset_rendering::{
...
#[cfg(test)]
mod tests {
    use inset_embedder::{Clip, Color, Offset, SceneBuilder, Size, TextDirection};
    use inset_foundation::AppCell;
    use inset_painting::{Border, BorderSide, BorderStyle, BoxDecoration};
    use inset_rendering::{
    ...
    fn a_clip_behavior_clips_the_decoration_through_its_clip_path() {
                ...
                Container::new()
                    .decoration(
                        BoxDecoration::new()
                            ...
                            .border_radius(inset_painting::BorderRadiusGeometry::circular(6.0)),
                    )
        ...
        let decorated = harness
            ...
            .as_object()
            .downcast::<inset_rendering::RenderDecoratedBox>(&app)
            .expect("the background DecoratedBox");
        let clip = decorated
            .child(&app)
            .expect("the clip under the decoration")
            .as_object()
            .downcast::<inset_rendering::RenderClipPath>(&app)
            .expect("a RenderClipPath");
        assert_eq!(
            inset_rendering::RenderCustomClip::clip_behavior(clip, &app),
            Clip::AntiAlias
        );
    }
}
