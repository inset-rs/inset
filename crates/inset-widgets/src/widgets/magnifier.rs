...
use inset_animation::AnimationController;
use inset_embedder::{Offset, Rect};
use inset_foundation::{App, Handle, Listener, ValueNotifier};
use inset_scheduler::TickerFuture;
...
impl MagnifierController {
    ...
    /// [`inset_animation::AnimationStatus::Dismissed`] state, or when
    /// the [`animation_controller`](Self::animation_controller) is animating out (i.e. in the
    /// [`inset_animation::AnimationStatus::Reverse`] state).
    ///
    ...
    /// [`inset_animation::AnimationStatus::Completed`] state, or in the
    /// [`inset_animation::AnimationStatus::Forward`]
    /// state.
