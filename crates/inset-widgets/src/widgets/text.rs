...
use inset_embedder::{FontWeight, TextAlign, TextDirection, TextHeightBehavior};
use inset_foundation::App;
use inset_painting::{
    InlineSpanRef, TextOverflow, TextScaler, TextSpan, TextStyle, TextWidthBasis,
};
use inset_rendering::{AnyRenderObject, RenderBox, RenderHandle, RenderParagraph};
...
pub struct Text {
    ...
    /// The text to display as an [`InlineSpan`](inset_painting::InlineSpan).
    ///
...
impl Text {
    ...
    /// Creates a text widget with an [`InlineSpan`](inset_painting::InlineSpan).
    ///
...
mod tests {
    ...
    use inset_embedder::{Color, TextLeadingDistribution};
