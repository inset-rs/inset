...
use inset_embedder::{Clip, Matrix4, Offset, Size, TextDirection};
use inset_foundation::{App, Handle, ListenableObject, Listener, ValueNotifier};
...
///
/// `Handle<OverlayEntry>` is a [`Listenable`](inset_foundation::Listenable) that notifies
...
impl ContainerBoxParentData for TheaterParentData {
    fn box_parent_data(&self) -> &inset_rendering::BoxParentData {
        ContainerBoxParentData::box_parent_data(&self.stack)
    }

    fn box_parent_data_mut(&mut self) -> &mut inset_rendering::BoxParentData {
...
impl RenderTheater {
    ...
    // `inset-rendering`, so the Overlay is relaid out instead (see `PORTING.md`).
...
impl RenderDeferredLayoutBox {
    ...
    fn do_layout_from(
        ...
        // `RenderObject`'s dirty flag is not readable outside `inset-rendering` (see
...
mod tests {
    ...
    fn overlay_layout_builder_resizes_without_requesting_child_dry_layout() {
        ...
        for _ in 0..2 {
            ...
            assert_eq!(
                inset_painting::transform_point(&info.child_paint_transform(), Offset::ZERO),
