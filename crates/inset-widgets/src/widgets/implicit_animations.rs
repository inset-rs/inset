...
use inset_rendering::{AlignmentGeometryTween, BoxConstraints, Constraints};
...
///
/// Unlike the tweens in `inset_animation`, this one is a value rather than an arena object:
...
macro_rules! implicitly_animated_widget_setters {
    ($name:ident) => {
        ...
        /// Dart `curve:`.
        pub fn curve(mut self, curve: ::std::rc::Rc<dyn ::inset_animation::Curve>) -> $name {
            self.implicitly_animated_widget.curve = curve;
            self
        }

        /// Dart `onEnd:`.
        pub fn on_end(mut self, on_end: ::inset_foundation::Listener) -> $name {
...
///
/// Implemented both by `Handle<Tween<T>>`, the arena tween of `inset_animation`, and by the
...
mod tests {
    ...
    use inset_rendering::{RenderAnimatedOpacity, RenderAnimatedOpacityMixin};
    ...
    fn render_box_of(app: &mut App, key: &GlobalKey) -> inset_rendering::AnyRenderBox {
