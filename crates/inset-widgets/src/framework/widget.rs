...
pub trait ParentDataWidget: Debug + 'static {
    ...
    fn debug_is_valid_render_object(
        &self,
        app: &App,
        render_object: ::inset_rendering::AnyRenderObject,
    ) -> bool {
...
impl GlobalKey {
    ...
    /// object is not an `S`.
    pub fn current_state<S: State>(&self, app: &mut App) -> Option<inset_foundation::Handle<S>> {
        let element = self.current_element(app)?;
