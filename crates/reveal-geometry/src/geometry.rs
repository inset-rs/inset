...
impl Radius {
    /// Constructs a circular radius. [`x`](Radius::x) and [`y`](Radius::y) will
    /// have the same radius value.
    pub const fn circular(radius: f64) -> Radius {
        Radius::elliptical(radius, radius)
    }

    /// Constructs an elliptical radius with the given radii.
    pub const fn elliptical(x: f64, y: f64) -> Radius {
        Radius { x, y }
    }

    /// A radius with [`x`](Radius::x) and [`y`](Radius::y) values set to zero.
    ///
    /// You can use [`Radius::ZERO`] with `RRect` or `RSuperellipse` to have
    /// right-angle corners.
    pub const ZERO: Radius = Radius::circular(0.0);

    /// Returns this [`Radius`], with values clamped to the given min and max
    /// [`Radius`] values.
    ///
    /// The `minimum` value defaults to `Radius::circular(f64::NEG_INFINITY)`,
    /// and the `maximum` value defaults to `Radius::circular(f64::INFINITY)`.
    pub fn clamp(&self, minimum: Option<Radius>, maximum: Option<Radius>) -> Radius {
        let minimum = minimum.unwrap_or(Radius::circular(f64::NEG_INFINITY));
        let maximum = maximum.unwrap_or(Radius::circular(f64::INFINITY));
        Radius::elliptical(
            clamp_double(self.x, minimum.x, maximum.x),
            clamp_double(self.y, minimum.y, maximum.y),
        )
    }
    ...
    ///
    /// The `minimum_x` and `minimum_y` values default to [`f64::NEG_INFINITY`],
    /// and the `maximum_x` and `maximum_y` values default to [`f64::INFINITY`].
    pub fn clamp_values(
        &self,
        minimum_x: Option<f64>,
        minimum_y: Option<f64>,
        maximum_x: Option<f64>,
        maximum_y: Option<f64>,
    ) -> Radius {
        Radius::elliptical(
            clamp_double(
                self.x,
                minimum_x.unwrap_or(f64::NEG_INFINITY),
                maximum_x.unwrap_or(f64::INFINITY),
            ),
            clamp_double(
                self.y,
                minimum_y.unwrap_or(f64::NEG_INFINITY),
                maximum_y.unwrap_or(f64::INFINITY),
            ),
        )
    }
    ...
    /// Integer (truncating) division operator.
    ///
    /// Returns a radius whose coordinates are the coordinates of this radius
    /// divided by the scalar `operand`, rounded towards zero.
    pub fn truncating_div(&self, operand: f64) -> Radius {
        Radius::elliptical((self.x / operand).trunc(), (self.y / operand).trunc())
    }
    ...
    /// Values for `t` are usually obtained from an `Animation<double>`, such as
    /// an `AnimationController`.
    pub fn lerp(a: Option<Radius>, b: Option<Radius>, t: f64) -> Option<Radius> {
        match (a, b) {
            (None, None) => None,
            (Some(a), None) => {
                let k = 1.0 - t;
                Some(Radius::elliptical(a.x * k, a.y * k))
            }
            (None, Some(b)) => Some(Radius::elliptical(b.x * t, b.y * t)),
            (Some(a), Some(b)) => Some(Radius::elliptical(
                lerp_double_non_null(a.x, b.x, t),
                lerp_double_non_null(a.y, b.y, t),
            )),
        }
    }
}
...
/// Binary subtraction operator.
///
/// Returns a radius whose [`x`](Radius::x) value is the left-hand-side
/// operand's [`x`](Radius::x) minus the right-hand-side operand's
/// [`x`](Radius::x) and whose [`y`](Radius::y) value is the left-hand-side
/// operand's [`y`](Radius::y) minus the right-hand-side operand's
/// [`y`](Radius::y).
impl Sub for Radius {
    type Output = Radius;
...
/// Binary addition operator.
///
/// Returns a radius whose [`x`](Radius::x) value is the sum of the
/// [`x`](Radius::x) values of the two operands, and whose [`y`](Radius::y)
/// value is the sum of the [`y`](Radius::y) values of the two operands.
impl Add for Radius {
    type Output = Radius;
...
/// Multiplication operator.
///
/// Returns a radius whose coordinates are the coordinates of the left-hand-side
/// operand (a radius) multiplied by the scalar right-hand-side operand (an
/// `f64`).
impl Mul<f64> for Radius {
    type Output = Radius;

    fn mul(self, operand: f64) -> Radius {
        Radius::elliptical(self.x * operand, self.y * operand)
    }
}
...
///
/// Returns a radius whose coordinates are the coordinates of the left-hand-side
/// operand (a radius) divided by the scalar right-hand-side operand (an `f64`).
impl Div<f64> for Radius {
    type Output = Radius;

    fn div(self, operand: f64) -> Radius {
        Radius::elliptical(self.x / operand, self.y / operand)
    }
}
...
impl Rem<f64> for Radius {
    ...
    fn rem(self, operand: f64) -> Radius {
        Radius::elliptical(self.x.rem_euclid(operand), self.y.rem_euclid(operand))
    }
}
...
impl Debug for Radius {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.x == self.y {
            write!(f, "Radius.circular({:.1})", self.x)
        } else {
            write!(f, "Radius.elliptical({:.1}, {:.1})", self.x, self.y)
        }
    }
}
...
mod tests {
    ...
    // engine geometry_test.dart 'Radius.clamp() operates as expected'.
    #[test]
    fn radius_clamp_operates_as_expected() {
        assert_eq!(
            Radius::circular(-100.0).clamp(Some(Radius::ZERO), None),
            Radius::circular(0.0)
        );
        assert_eq!(
            Radius::circular(100.0).clamp(None, Some(Radius::circular(10.0))),
            Radius::circular(10.0)
        );
        assert_eq!(
            Radius::elliptical(-100.0, 100.0)
                .clamp(Some(Radius::ZERO), Some(Radius::circular(10.0))),
            Radius::elliptical(0.0, 10.0)
        );
        assert_eq!(
            Radius::elliptical(100.0, -100.0)
                .clamp(Some(Radius::ZERO), Some(Radius::circular(10.0))),
    ...
    // engine geometry_test.dart 'Radius.clampValues() operates as expected'.
    #[test]
    fn radius_clamp_values_operates_as_expected() {
        assert_eq!(
            Radius::circular(-100.0).clamp_values(Some(0.0), Some(0.0), None, None),
            Radius::circular(0.0)
        );
        assert_eq!(
            Radius::circular(100.0).clamp_values(None, None, Some(10.0), Some(20.0)),
            Radius::elliptical(10.0, 20.0)
        );
        assert_eq!(
            Radius::elliptical(-100.0, 100.0).clamp_values(
                Some(5.0),
                Some(6.0),
                Some(10.0),
                Some(20.0)
            ),
            Radius::elliptical(5.0, 20.0)
        );
        assert_eq!(
            Radius::elliptical(100.0, -100.0).clamp_values(
                Some(5.0),
            ...
            Radius::elliptical(10.0, 6.0)
        );
    }

    #[test]
    fn radius_arithmetic_is_component_wise() {
        let a = Radius::elliptical(10.0, 20.0);
        let b = Radius::elliptical(1.0, 2.0);

        assert_eq!(a + b, Radius::elliptical(11.0, 22.0));
        assert_eq!(a - b, Radius::elliptical(9.0, 18.0));
        assert_eq!(a * 2.0, Radius::elliptical(20.0, 40.0));
        assert_eq!(a / 2.0, Radius::elliptical(5.0, 10.0));
        assert_eq!(-a, Radius::elliptical(-10.0, -20.0));
        assert_eq!(a.truncating_div(3.0), Radius::elliptical(3.0, 6.0));
        assert_eq!(
            Radius::elliptical(-5.0, 5.0) % 3.0,
            Radius::elliptical(1.0, 2.0)
        );
        assert_eq!(
            Radius::elliptical(-5.0, 5.0).truncating_div(3.0),
            Radius::elliptical(-1.0, 1.0)
        );
    }

    #[test]
    fn radius_circular_equals_elliptical_when_axes_match() {
        assert_eq!(Radius::circular(5.0), Radius::elliptical(5.0, 5.0));
        assert_ne!(Radius::circular(5.0), Radius::elliptical(5.0, 6.0));
        assert_eq!(Radius::ZERO, Radius::circular(0.0));
    }

    #[test]
    fn radius_lerp_defaults_a_missing_end_to_zero() {
        let a = Radius::elliptical(0.0, 10.0);
        let b = Radius::elliptical(10.0, 0.0);

        assert_eq!(
            Radius::lerp(Some(a), Some(b), 0.5),
            Some(Radius::elliptical(5.0, 5.0))
        );
        assert_eq!(
            Radius::lerp(Some(a), None, 0.5),
            Some(Radius::elliptical(0.0, 5.0))
        );
        assert_eq!(
            Radius::lerp(None, Some(b), 0.5),
            Some(Radius::elliptical(5.0, 0.0))
        );
        assert_eq!(Radius::lerp(None, None, 0.5), None);
    }

    #[test]
    fn radius_debug_matches_darts_to_string() {
        assert_eq!(
            format!("{:?}", Radius::circular(1.5)),
            "Radius.circular(1.5)"
        );
        assert_eq!(
            format!("{:?}", Radius::elliptical(1.0, 2.5)),
            "Radius.elliptical(1.0, 2.5)"
        );
    }
}
