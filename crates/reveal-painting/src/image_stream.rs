...
use reveal_embedder::{Image, ImageDecodeError};
...
impl ImageInfo {
    ...
    pub fn size(&self) -> reveal_embedder::Size {
        let [width, height] = self.image.size();
        reveal_embedder::Size::new(
...
mod tests {
    ...
    fn an_image_reports_its_logical_size_and_what_it_costs_to_hold() {
        ...
        assert_eq!(info.size(), reveal_embedder::Size::new(2.0, 1.0));
