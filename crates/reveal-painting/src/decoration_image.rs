...
use reveal_embedder::{Canvas, Image, Offset, Paint, Rect, Size};
...
fn valo_color_with_alpha(opacity: f64) -> reveal_embedder::valo::Color {
    let alpha = opacity.clamp(0.0, 1.0) as f32;
    reveal_embedder::valo::Color::rgba(alpha, alpha, alpha, alpha)
}
