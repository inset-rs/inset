...
use inset_rendering::{BoxHitTestResult, CrossAxisAlignment, HitTestBehavior};
use inset_services::{
...
impl CupertinoTextFieldState {
    ...
    fn create_local_controller(
        self: Handle<Self>,
        app: &mut App,
        value: Option<inset_services::TextEditingValue>,
    ) {
    ...
    fn build_clear_button(self: Handle<Self>, app: &mut App, context: BuildContext) -> WidgetRef {
        ...
        let icon = inset_widgets::Icon::new(Some(CupertinoIcons::clear_thick_circled()))
            .size(18.0)
            .color(color);
        let _ = enabled;
        GestureDetector::new()
...
impl RestorationMixin for CupertinoTextFieldState {
    inset_widgets::restoration_mixin_accessors!();
...
fn resolve_box_border(
    border: Option<&dyn inset_painting::BoxBorder>,
    app: &mut App,
    context: BuildContext,
) -> Option<Border> {
    let border = border?;
...
mod tests {
    ...
    use inset_widgets::{AnyElement, Column, Expanded, Listener, SizedBox, WidgetsBinding};
