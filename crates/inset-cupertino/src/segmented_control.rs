...
use inset_services::{MouseCursor, SystemMouseCursors};
use inset_widgets::{
...
impl<T: Copy + Eq + Hash + Debug + 'static> RadioClient<T> for SegmentButtonState<T> {
    inset_widgets::radio_client_accessors!(T);
...
mod tests {
    ...
    use inset_widgets::{GlobalKey, SizedBox, WidgetsBinding};
    ...
    impl State for HostState {
        ...
        fn build(self: Handle<Self>, app: &mut App, _context: BuildContext) -> WidgetRef {
            ...
            inset_widgets::Directionality::new(self.widget(app).text_direction, control)
                .into_widget()
        }
    }
