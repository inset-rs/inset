...
use inset_embedder::{Brightness, Size, TargetPlatform};
...
use inset_services::{MouseCursor, MouseCursorRef, SystemMouseCursors};
use inset_widgets::{
...
impl CupertinoButtonState {
    ...
    fn render_box(self: Handle<Self>, app: &App) -> inset_rendering::AnyRenderBox {
        self.context(app)
...
mod tests {
    ...
    fn a_mouse_click_on_a_filled_text_button_in_a_dark_theme() {
        ...
        let button = CupertinoButton::filled(
            inset_widgets::Text::new("Press me").into_widget(),
            Some(on_pressed),
        );
        ...
        let tree = Directionality::new(
            ...
            DecoratedBox::new(inset_painting::BoxDecoration::new()).child(
                Padding::new(EdgeInsetsGeometry::all(24.0)).child(
                    inset_widgets::Center::new().child(
                        SizedBox::new()
                            .width(240.0)
                            .height(140.0)
                            .child(CupertinoTheme::new(
                                ...
                                inset_widgets::Center::new().child(button),
                            )),
                    ),
                ),
            ),
