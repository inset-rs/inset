...
use inset_gestures::{GestureBinding, HitTestResult};
use inset_painting::{AnyColor, BoxDecoration, EdgeInsetsGeometry};
use inset_rendering::{BoxHitTestEntry, HitTestBehavior};
use inset_widgets::{
...
impl CupertinoPageScaffoldState {
    ...
    pub fn handle_status_bar_tap(self: Handle<Self>, app: &mut App) {
        ...
        let Some(primary_scroll_controller) =
            inset_widgets::PrimaryScrollController::maybe_of(app, context)
        else {
            return;
        };
...
mod tests {
    ...
    use inset_rendering::{AnyRenderObject, RenderPadding};
    use inset_widgets::{Directionality, MediaQueryData};
    ...
    impl inset_widgets::StatelessWidget for TestNavigationBar {
        fn build(&self, _app: &mut App, _context: BuildContext) -> WidgetRef {
    ...
    fn mount(
        ...
        let root = inset_widgets::WidgetsBinding::instance(&mut app)
            .root_element(&app)
