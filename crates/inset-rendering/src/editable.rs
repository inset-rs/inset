...
use inset_foundation::{App, Handle, Listenable, Listener, ValueListenable, ValueNotifier};
use inset_gestures::TapDownDetails;
use inset_painting::{
    ClipContext, InlineSpanRef, PaintingBinding, TextPainter, TextScaler, TextWidthBasis,
};
use inset_services::{AnyTextSelectionDelegate, SelectionChangedCause, TextLayoutMetrics};
...
impl RenderEditable {
    ...
    /// The [`TextSelectionDelegate`](inset_services::TextSelectionDelegate) for
    /// this [`RenderEditable`].
    ...
    fn set_text_editing_value(
        self: RenderHandle<Self>,
        app: &mut App,
        new_value: inset_embedder::TextEditingValue,
    ...
    fn paint_highlights(
        self: RenderHandle<Self>,
        app: &App,
        canvas: &mut inset_embedder::Canvas,
        offset: Offset,
    ) {
...
mod tests {
    ...
    use inset_embedder::{FontSource, TextAffinity};
    use inset_foundation::{AppCell, ValueListenable};
    use inset_painting::{PaintingBinding, TextSpan, TextStyle};
    use inset_scheduler::SchedulerBinding;
    use inset_services::{TextEditingValue, TextSelectionDelegate};
