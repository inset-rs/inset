...
//!
//! The value types live in `inset-embedder` because [`inset_embedder::View`]
//! and [`inset_embedder::EmbedderClient`] name them. This file is the
...
pub trait TextSelectionDelegate: Sized + 'static {
    ...
    /// input.
    fn bring_into_view(self: Handle<Self>, app: &mut App, position: inset_embedder::TextPosition) {
...
struct TextSelectionDelegateVTable {
    ...
    bring_into_view: fn(&mut App, HandleId, inset_embedder::TextPosition),
...
impl AnyTextSelectionDelegate {
    ...
    pub fn bring_into_view(self, app: &mut App, position: inset_embedder::TextPosition) {
...
#[cfg(test)]
mod tests {
    use inset_embedder::TextAffinity;
